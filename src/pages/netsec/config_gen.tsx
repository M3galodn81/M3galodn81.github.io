...
export default function DeviceConfiguration() {
  ...
  const [consolePassword, setConsolePassword] = useState(""); // User EXEC
  ...
  const [interfaces, setInterfaces] = useState<Iface[]>([
    { name: "GigabitEthernet0/1", type: "access", vlan: 10, security: true, maxMacs: 2, violation: 'restrict', description: "User PC" },
    { name: "GigabitEthernet0/24", type: "trunk", allowed: "10,20,99", description: "Uplink to Core", dhcpTrust: true, arpTrust: true }
  ]);
  ...
  const addInterface = () => setInterfaces([...interfaces, { name: "FastEthernet0/1", type: "access", vlan: 10, violation: 'restrict' }]);
  ...
  return (
    <div className={`min-h-screen w-screen flex flex-col items-center p-8 ${uiBackground} animate-in fade-in duration-700`}>
      
      {/* Header */}
      <div className="max-w-7xl w-full mb-8 flex justify-between items-end">
        <div>
          <h1 className="text-4xl font-bold text-slate-800 dark:text-white mb-2">Config Generator</h1>
          <p className="text-slate-600 dark:text-slate-300 text-lg">Automate your Cisco IOS switch setup.</p>
        </div>
        <Link to="/" className="text-indigo-600 dark:text-indigo-400 font-medium hover:underline mb-2 flex items-center gap-2">
           ...
           <div className={bentoCard}>
             <Tabs defaultValue="general">
               <TabsList className="grid w-full grid-cols-4 mb-6">
                 ...
                 <TabsTrigger value="interfaces" className="flex items-center gap-2"><Terminal className="w-4 h-4" /> Interfaces</TabsTrigger>
               ...
               <TabsContent value="general" className="space-y-4">
                 <div className="grid grid-cols-2 gap-4">
                   <div className="space-y-2">
                     ...
                     <Input value={hostname} onChange={(e) => setHostname(e.target.value)} className="bg-white/50 dark:bg-black/20" />
                   </div>
                   <div className="space-y-2">
                     <Label>Banner Message</Label>
                     <Input value={banner} onChange={(e) => setBanner(e.target.value)} className="bg-white/50 dark:bg-black/20" />
                   </div>
                   <div className="space-y-2">
                     <Label>Enable Secret (Privileged)</Label>
                     <Input type="password" value={enableSecret} onChange={(e) => setEnableSecret(e.target.value)} className="bg-white/50 dark:bg-black/20" placeholder="Optional" />
                   </div>
                   <div className="space-y-2">
                     <Label>Console Password (User EXEC)</Label>
                     <Input type="password" value={consolePassword} onChange={(e) => setConsolePassword(e.target.value)} className="bg-white/50 dark:bg-black/20" placeholder="Optional" />
                   </div>
                 </div>
                    ...
                    <div>
                        ...
                        <div className="space-y-3">
                            <div className="space-y-1">
                                ...
                                <Input value={domain} onChange={(e) => setDomain(e.target.value)} className="h-8 bg-white/50 dark:bg-black/20" />
                            </div>
                            <div className="space-y-1">
                                <Label className="text-xs">Username</Label>
                                <Input value={username} onChange={(e) => setUsername(e.target.value)} className="h-8 bg-white/50 dark:bg-black/20" />
                            </div>
                            <div className="space-y-1">
                                <Label className="text-xs">Password</Label>
                                <Input type="password" value={password} onChange={(e) => setPassword(e.target.value)} className="h-8 bg-white/50 dark:bg-black/20" />
                            </div>
                        </div>
                    </div>
                    <div>
               ...
               <TabsContent value="vlans" className="space-y-4">
                 {vlans.map((vlan, idx) => (
                   <div key={idx} className="flex gap-4 items-end bg-slate-50 dark:bg-white/5 p-3 rounded-xl border border-slate-100 dark:border-white/5">
                     <div className="w-20 space-y-1">
                        <Label className="text-xs">ID</Label>
                        <Input type="number" value={vlan.id} onChange={(e) => updateVlan(idx, 'id', parseInt(e.target.value))} className="h-8" />
                     </div>
                     <div className="flex-1 space-y-1">
                        <Label className="text-xs">Name</Label>
                        <Input value={vlan.name} onChange={(e) => updateVlan(idx, 'name', e.target.value)} className="h-8" />
                     </div>
                     <div className="w-32 space-y-1">
                        <Label className="text-xs">SVI IP (Opt)</Label>
                        <Input value={vlan.ip || ''} onChange={(e) => updateVlan(idx, 'ip', e.target.value)} className="h-8" placeholder="192.168.1.1" />
                     </div>
                     <Button variant="destructive" size="icon" className="h-8 w-8" onClick={() => removeVlan(idx)}><Trash2 className="w-4 h-4" /></Button>
                   </div>
                 ))}
               ...
               <TabsContent value="interfaces" className="space-y-4">
                 {interfaces.map((iface, idx) => (
                   <div key={idx} className="flex flex-col gap-3 bg-slate-50 dark:bg-white/5 p-4 rounded-xl border border-slate-100 dark:border-white/5">
                      <div className="flex gap-3">
                        <div className="w-32 space-y-1">
                            <Label className="text-xs">Interface</Label>
                            <Input value={iface.name} onChange={(e) => updateInterface(idx, 'name', e.target.value)} className="h-8" placeholder="Gi0/1" />
                        </div>
                        <div className="flex-1 space-y-1">
                            <Label className="text-xs">Description</Label>
                            <Input value={iface.description || ''} onChange={(e) => updateInterface(idx, 'description', e.target.value)} className="h-8" placeholder="e.g. Printer Connection" />
                        </div>
                        ...
                        <Button variant="destructive" size="icon" className="h-8 w-8 mt-5" onClick={() => removeInterface(idx)}><Trash2 className="w-4 h-4" /></Button>
                      </div>
                             ...
                             {iface.type === 'access' ? (
                                <div className="flex gap-2">
                                    <div className="w-20 space-y-1">
                                        ...
                                        <Input type="number" value={iface.vlan} onChange={(e) => updateInterface(idx, 'vlan', parseInt(e.target.value))} className="h-7 text-xs" />
                                    </div>
                                    <div className="flex-1 space-y-1">
                                ...
                                <div className="space-y-1">
                                    ...
                                    <Input value={iface.allowed || ''} onChange={(e) => updateInterface(idx, 'allowed', e.target.value)} className="h-7 text-xs" placeholder="10,20,30" />
                                </div>
                             )}

                             {/* Port Security Detail */}
                             {iface.type === 'access' && iface.security && (
                                 <div className="flex gap-2 pl-4 border-l-2 border-slate-200">
                                    <div className="w-20 space-y-1">
                                        <Label className="text-[10px]">Max MACs</Label>
                                        <Input type="number" value={iface.maxMacs || 1} onChange={(e) => updateInterface(idx, 'maxMacs', parseInt(e.target.value))} className="h-7 text-xs" />
                                    </div>
                                    <div className="flex-1 space-y-1">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                      <div className="space-y-2">
                        ...
                        <Input 
                            ...
                            className={`bg-white/50 dark:bg-black/20 ${errors.gateway ? 'border-red-500 focus-visible:ring-red-500' : ''}`} 
                        />
                      </div>
                      <div className="space-y-2">
                        <div className="flex justify-between">
                        ...
                        <Input 
                            ...
                            className={`bg-white/50 dark:bg-black/20 ${errors.ntp ? 'border-red-500 focus-visible:ring-red-500' : ''}`} 
                        />
                      </div>
                      <div className="space-y-2">
                        <Label>SNMP Community</Label>
                        <Input value={snmp} onChange={(e) => setSnmp(e.target.value)} placeholder="public" className="bg-white/50 dark:bg-black/20" />
                      </div>
                      <div className="space-y-2">
                        <div className="flex justify-between">
                        ...
                        <Input 
                            ...
                            className={`bg-white/50 dark:bg-black/20 ${errors.logging ? 'border-red-500 focus-visible:ring-red-500' : ''}`} 
                        />
                      </div>
                  </div>
               </TabsContent>
