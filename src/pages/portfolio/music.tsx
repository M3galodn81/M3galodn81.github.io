...
import { Check, X, AlertCircle, ExternalLink, Clock, Activity } from "lucide-react";
...
export default function Music() {
  ...
  const groupedTracks = useMemo(() => {
    ...
    // Sort tracks by date desc first
    ...
    // Return entries sorted by year desc
              ...
              <table className="w-full text-left text-sm text-slate-600 dark:text-slate-400">
                <thead className="bg-slate-100/50 dark:bg-slate-800/50 text-xs uppercase font-semibold text-slate-500 dark:text-slate-400">
                  <tr>
                    ...
                    {/* <th className="px-6 py-4 text-center">Video Use</th> */}
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100 dark:divide-white/5">
                  {groupedTracks.map(([year, yearTracks]) => (
                    <>
                      {/* Year Row */}
                      <tr key={`year-${year}`} className="bg-slate-50/50 dark:bg-white/5">
                        <td colSpan={5} className="px-6 py-3 font-bold text-slate-800 dark:text-white text-lg">
                          {year}
                        </td>
                      </tr>
                      {/* Track Rows */}
                      {yearTracks.map((track) => (
                        ...
                        >
                          <td className="px-6 py-4">
                            <div className="font-medium text-slate-900 dark:text-white group-hover:text-orange-500 transition-colors flex items-center gap-2">
                              {track.title}
                          ...
                          {/* <td className="px-6 py-4 text-center">
                            <PermissionIcon allowed={track.permissions.use_in_videos} />
                          </td> */}
                        </tr>
                      ))}
                    </>
